...
**Phase 10am — COMPLETE** — Fix video node controls clipping at all zoom levels: `MediaOverlay.svelte` had `overflow: hidden` on the `.media-overlay-item` container — at certain zoom levels this clipped the `vc-footer` controls bar; fixed by adding `.no-clip` modifier class for video elements that sets `overflow: visible`, letting `VideoPlayer.svelte`'s own `video-card { overflow: hidden }` handle clipping and border-radius instead. v0.7.68.

**Phase 10al — COMPLETE** — Video node rewrite + screenshot fix + hover/connection line fixes: `VideoPlayer.svelte` rewritten with flex column layout (video area `flex:1` + `vc-footer flex-shrink:0`); `onLoaded` auto-resizes element to native aspect ratio via `elementsStore` update (adds `FOOTER_H=30` world units for footer); screenshot uses a hidden `<video>` element to seek to current frame and `canvas.toBlob` → `placeImageBlob`; hover and connection line rendering fixes in `Canvas.svelte`. v0.7.67.

**Phase 10ak — COMPLETE** — Fix SQLite never initialized: `initDB()` was never called — in Tauri mode all persistence silently fell back to localStorage; `initProjectsService` now calls `initDB()` on startup when `IS_TAURI_STORAGE`, then `dbGetAllSettings/dbLoadProjects/dbLoadFolders` in parallel, calls `markDbReady(settings)` and seeds projects/folders stores from SQLite; browser mode unchanged (localStorage path); `markDbReady` removed from `window` bridge (now internal); stale `_dbReady`/`_memStore` window getters removed (those vars moved to kv-store.js). v0.7.66.

**Phase 10aj — COMPLETE** — Break circular import `canvas-persistence.js` ↔ `projects-service.js`: extracted `store`, `markDbReady`, `setStorageFullCallback` into new `lib/kv-store.js`; `kv-store.js` imports only `db.js`; `canvas-persistence.js` now imports `store` from `kv-store.js` (no cycle); `projects-service.js` imports from `kv-store.js` and re-exports `store` for other consumers; `setStorageFullCallback` wired to `showToast` immediately after showToast is defined; `dbSet`/`dbRemove` removed from projects-service.js db import (now in kv-store.js). v0.7.65.

**Phase 10ai — COMPLETE** — Eliminate `window.store` reads: exported `store` from `projects-service.js`; all 11 `window.store?.get/set` call sites replaced with direct `store.get/set` — `AINotePanel.svelte` (get key, set key), `BrainstormPanel.svelte` (get claude key), `BookmarkPanel.svelte` (load/persist bookmarks), `canvas-actions.js` (get claude key), `canvas-persistence.js` (2× get filepath), `FolderBrowser.svelte` (load/save project dir), `projects-service.js` (self-call in `_tryMigrateV1`); `store` still exposed on `window` for any legacy code. v0.7.64.

**Phase 10ah — COMPLETE** — Viewport cold-load timing fix: `Canvas.svelte` local vars `scale`, `px`, `py` were initialized to hardcoded `1, 0, 0` — so if `applyCanvasState` pre-seeded the stores before Canvas mounted, `onMount → applyViewport()` would still use `1/0/0`; fixed by initializing from stores: `let scale = get(scaleStore) || 1; let px = get(pxStore) || 0; let py = get(pyStore) || 0`; `get` imported from `svelte/store`. v0.7.63.

**Phase 10ag — COMPLETE** — ExportPanel.svelte + summariseCanvas: `ExportPanel.svelte` created with 4 export options (JSON, Markdown, PNG, Share bundle); mounts at body level in `main.js`; `CanvasBar.svelte` export button now passes event `e` to `window.openExportPanel(e)` for correct positioning; `summariseCanvas()` added to `canvas-actions.js` — reads notes/todos/labels from `elementsStore`, calls `callAI('claude', ...)` with canvas context, shows result in toast. PDF/EXR metadata now persisted to element store: `placeExrBlob` and `placePdf` in `media-service.js` update `elementsStore` via `card._elId` after `makeImgCard` returns (previously set on proxy `dataset`, never reached the store). v0.7.63.

**Phase 10af — COMPLETE** — Dead code removal + Claude API key fix: `callClaude` in `ai-service.js` never sent `x-api-key` header — all Claude calls from both `AINotePanel` and `BrainstormPanel` would 401 in production; `callClaude` now accepts `apiKey` param and sends it as `x-api-key`; `callAI` now passes key to `callClaude` (was dropped); `BrainstormPanel` switched from `callClaude` to `callAI('claude', ..., key)` reading `freeflow_key_claude` from store (same pattern as AINotePanel). Dead stubs removed: `renderBookmarkList` from `main.js`; `card.dataset.isExr` from `placeExrBlob`; `card.dataset.pdfPage`/`pdfName` from `placePdf` — set on proxy but never read anywhere. v0.7.62.

**Phase 10ae — COMPLETE** — Eliminate `window._projectDir` bridge: `media-service.js` imports `projectDirStore` from `stores/ui.js` directly; `saveImgToProjectDir` now reads `_getStore(projectDirStore)` instead of `window._projectDir`; all 4 call-sites that pre-checked `window._projectDir` before calling `saveImgToProjectDir` simplified (guard removed — function handles null internally); `FolderBrowser.svelte` `loadProjectDir` and `saveProjectDir` no longer write to `window._projectDir`. v0.7.61.

**Phase 10ad — COMPLETE** — Fix note/todo/ai-note placement computing wrong world position: `Toolbar.svelte` and `CommandPalette.svelte` were using `window._pixiApp.screen.width/2 + 3000` — this only gives the correct world position at default viewport (scale=1, no pan); replaced with `window.c2w(window.innerWidth/2, window.innerHeight/2)` which correctly accounts for current scale and pan. `main.js` `_screenCenter` had the same issue — simplified to call `_clientToWorld(window.innerWidth/2, window.innerHeight/2, {left:0,top:0})`; removed `window._pixiApp` dependency from `_screenCenter`. `main.js` dblclick handler `getBoundingClientRect()` on `#cv` replaced with hardcoded `{left:0,top:0}` (canvas is always full-viewport, rect is always zero). v0.7.60.

**Phase 10ac — COMPLETE** — Replace `body.on-canvas` class reads with `isOnCanvasStore`: added `isOnCanvasStore` to `stores/ui.js`; `projects-service.js` sets it `true` in `openProject` and `false` in `goToDashboard` alongside the existing CSS class (kept for styling); all 3 `classList.contains('on-canvas')` reads in `projects-service.js` and all 4 in `MediaDropHandler.svelte` replaced with store reads. `toggleTheme` in `canvas-actions.js` now reads `getIsLight()` instead of `document.body.classList.contains('light')`. `onMount` cleanup added to `SearchBox.svelte`, `TimerPanel.svelte`, `ClearConfirm.svelte`, `ImportPanel.svelte` (window globals now deleted on unmount). `window.projects` dead fallback removed from `ProjectGrid.svelte`. v0.7.59.

**Phase 10ab — COMPLETE** — Eliminate `window.selectedElIds` bridge and `getElementById('cv')`: `Canvas.svelte` was mirroring `selected` to `window.selectedElIds` via `$effect`; `canvas-actions.js` (`gridSelected`, `changeSelectedFontSize`, `togglePinSelected`, `toggleLockSelected`) and `FolderBrowser.svelte` (3 sites) read it — all replaced with `getSelected()` / `get(selectedStore)` directly; `window.selectedElIds` `$effect` removed from Canvas.svelte. `window.activeProjectId` read in `canvas-actions.js` replaced with `_getStore(activeProjectIdStore)`; in `FolderBrowser.svelte` replaced with `get(activeProjectIdStore)`. `getElementById('cv')` in `canvas-actions.js` `_screenCentre` and `media-service.js` `_canvasCentre` removed — both compute center from `window.innerWidth/2` + `window.innerHeight/2` (canvas always covers the viewport). v0.7.58.

**Phase 10aa — COMPLETE** — `CanvasBar.svelte` DOM cleanup: replaced `getElementById`-based win-controls wiring with `bind:this` refs (`winControls`, `winMin`, `winMax`, `winClose`); win button click handlers moved from `addEventListener` in `onMount` to inline `onclick` in template (with `IS_TAURI` guard and `e.stopPropagation()`); `onMount` now only sets up document-level drag (`mousedown`) and double-click-maximize (`dblclick`) listeners; removed `winOps` object. v0.7.57.

**Phase 10z — COMPLETE** — Fix theme system: `Canvas.svelte` was reading `document.body.classList.contains('light')` in 9 script functions and 1 template expression — replaced all with `$isLightStore` (imported `isLightStore` from stores); `isLightStore` subscription added so theme toggle triggers PixiJS re-render of elements and strokes; canvas theme was never restored on reload — `canvas-actions.js` now reads `localStorage.getItem('freeflow_theme')` at module init and calls `setIsLight(true)` + adds `body.light` class if 'light'. `BookmarkPanel.svelte` `$effect` for window globals replaced with `onMount` (correct — runs once, cleans up on destroy). v0.7.56.

**Phase 10y — COMPLETE** — Fix `blobURLCache` split-brain and viewport restore across `export-service.js` and `canvas-persistence.js`: both were using `window.blobURLCache` (a separate object from the module-level `blobURLCache` in `media-service.js`, so imported blobs were invisible to `getBlobURL`); both now import `blobURLCache` and `registerBlobURLs` from `media-service.js` directly; both were restoring viewport via `window._pixiCanvas.restorePixiCanvas` (full restore, not viewport-only); replaced with `window._applyViewportTo(scale, px, py)`. Wire `exportJSON`/`exportPNG`/`exportMarkdown`/`exportSharedCanvasV2` from `export-service.js` onto window in `main.js` (were "coming soon" stubs); remove dead `restoreImgCards`, `updateMinimap`, `triggerImg`/`onImgFiles`/`onPdfFiles` stubs; remove dead `setSnapEnabled`/`getSnapEnabled` from `stores/canvas.js`. v0.7.55.

**Phase 10x — COMPLETE** — Fix `window.c2w` and `window._projectDir` never assigned: `Canvas.svelte` now sets `window.c2w` in `onMount` (delegates to local `c2w()` which uses live `px`/`py`/`scale`); fallback in `media-service.js` and `MediaDropHandler.svelte` replaced with store-based calculation (was returning raw client coords — dropped media would land at wrong position); `FolderBrowser.svelte` now sets `window._projectDir` alongside `projectDir` local var and `projectDirStore` in both `loadProjectDir` and `saveProjectDir` (was never assigned, so `saveImgToProjectDir` silently skipped copying to project folder). `media-service.js` imports `scaleStore`/`pxStore`/`pyStore` for the fallback. v0.7.54.

**Phase 10w — COMPLETE** — Fix `exportSharedCanvas` broken by v2 migration: was gating on `freeflow_filepath_` (a v1 key never written in v2) so the ⬡ share button always failed with "Save your canvas to a file first"; replaced with a Tauri `save` dialog to pick the output `.json` path (same pattern as `saveCanvasToFile`), then derives the `_share` folder from that path. v0.7.53.

**Phase 10v — COMPLETE** — Fix `curTool`/`snapEnabled` store sync in `Canvas.svelte`: `window.tool(t)` and `_pixiCanvas.setTool(t)` were setting local `curTool` but not calling `setCurTool` — Toolbar and StatusBar read `$curToolStore` so tool buttons never lit up; fixed to call `setCurTool(t)` alongside local assignment. `toggleSnap` (via G key and `_pixiCanvas.toggleSnap`) toggled local `snapEnabled` but never updated `snapEnabledStore` — Toolbar snap button never showed active; fixed by adding `snapEnabledStore.set(snapEnabled)` to both toggle sites. Imported `snapEnabledStore` in Canvas.svelte. v0.7.52.

**Phase 10u — COMPLETE** — Fix two broken window globals: `ImageCard.svelte` was calling missing `window.getImgBlob` — replaced with direct import of `getBlobURL` from `media-service.js` (checks cache, loads from Tauri FS / IndexedDB, caches result); `Minimap.svelte` was calling `_applyViewportTo(wx, wy)` with 2 args but the function expects `(scale, px, py)` — fixed to compute proper viewport `px`/`py` from the clicked world point (`cx - wx*scale`, `cy - wy*scale`) and pass current `scale`. v0.7.51.

**Phase 10t — COMPLETE** — Fix missing window globals: `window.zoomToFit` and `window.zoomToSelection` were called from CanvasBar/CommandPalette/Toolbar/BrainstormPanel but never defined — Canvas.svelte now sets them in `onMount`; `window.undo`/`window.redo` called from CommandPalette but missing — added to `canvas-actions.js` window bridge (imported from `stores/elements.js`); dead stubs `toggleCmdPalette`/`openCmdPalette`/`closeCmdPalette` removed from `main.js` (CommandPalette manages its own state). v0.7.50.

**Phase 10s — COMPLETE** — Remove dead no-op window stubs from `canvas-actions.js` and wire `window.tool`: deleted `syncUndoButtons`, `updateSelBar`, `addRelHandle`, `startConnDrag`, `cleanupElConnections`, `loadViewBookmarks`, `saveViewBookmarks`, `setSnapEnabled`, `saveCanvasState`, `loadCanvasState`, `serializeCanvas`, `restoreCanvas` (none called anywhere); `Canvas.svelte` now sets `window.tool = (t) => { curTool = t; }` and exposes `setTool` on `_pixiCanvas` so toolbar tool buttons actually work. -14 lines. v0.7.49.

**Phase 10r — COMPLETE** — Fix `_applySharedCanvas` in `canvas-actions.js`: was using legacy `window.projects.push` + `window.saveProjects` + `window.store.set('freeflow_canvas_'+id)` + no-op `window.loadCanvasState` to import a shared canvas; replaced with `window.createProject` + `applyCanvasState` + `saveCanvasV2` + `window.openProject`; imports `saveCanvasV2` / `applyCanvasState` from `canvas-persistence.js`. v0.7.48.

**Phase 10q — COMPLETE** — Fix ProjectGrid.svelte delete/rename using bridge functions: `confirmDelete` was duplicating `deleteProject` logic (manual filter + `store.remove` + `dbDeleteProject` + `saveProjects` + toast) — replaced with single `window.deleteProject(id)` call; `commitRename` was mutating `p.name` directly and calling `window.saveProjects` — replaced with `window.renameProject(id, val)`; `renameProject` function added to `projects-service.js` (sets name, updatedAt, calls saveProjects) and exposed on window bridge. v0.7.47.

**Phase 10p — COMPLETE** — Remove dead Tauri auto-updater IIFE from `main.js` (35 lines): was directly invoking `plugin:updater|check` and writing to `#update-btn`/`#canvas-update-btn` DOM elements; `UpdateBanner.svelte` already handles all of this with the proper `@tauri-apps/plugin-updater` API and overrides `window.checkForAppUpdate`; `#canvas-update-btn` removed from `CanvasBar.svelte`; `_IS_TAURI` const removed; comment block left pointing to `UpdateBanner.svelte` and `CanvasBar.svelte`. v0.7.46.

**Phase 10o — COMPLETE** — Migrate `#dash-backdrop` opacity from JS mutation to pure CSS: `FolderBrowser.svelte` was calling `getElementById('dash-backdrop').style.opacity = '0/1'`; replaced with `projectDirStore` in `stores/ui.js`; `FolderBrowser.svelte` sets store, `CanvasBar.svelte` displays `$projectDirStore` basename reactively; `#dash-backdrop` CSS moved to `style.css` using `body.on-canvas` selector; `index.html` `#dash-backdrop` now bare `<div>`. v0.7.45.

**Phase 10n — COMPLETE** — Remove dead ctx-menu delegate functions from `canvas-actions.js`: `_openNoteMenu`, `_closeNoteMenu`, `deleteMenuNote`, `deleteMenuFrame`, `zoomToMenuNote`, `zoomToMenuFrame`, `togglePinNote`, `toggleLockNote`, `saveLink`, `_ctxMenuElId` — Canvas.svelte has its own built-in context menu; `toggleAlwaysOnTop` now sets `alwaysOnTopStore` instead of querying `#always-on-top-btn`; `CanvasBar.svelte` uses `class:active={$alwaysOnTopStore}` reactively. -33 lines. v0.7.44.

**Phase 10m — COMPLETE** — Migrate toast from DOM mutation to `Toast.svelte` + store: `toastMsgStore`/`toastVisibleStore` (already in `stores/ui.js`) now driven by `showToast()` in `projects-service.js`; `Toast.svelte` created with scoped CSS; mounted at body level in `main.js`; `#toast` div removed from `index.html`; `#toast` CSS rules removed from `style.css`. v0.7.43.

**Phase 10l — COMPLETE** — `index.html` reaches zero `onclick=` attributes: `#dash-backdrop onclick` removed and wired via `addEventListener` in `main.js`; stale comments consolidated; `index.html` is now 90 lines of pure structural HTML with no inline event handlers. v0.7.42.

**Phase 10k — COMPLETE** — Replace DOM-manipulation rename/delete with Svelte state in `ProjectGrid.svelte`: `renamingId`/`renameVal` state added; card-name div replaced with conditional inline `<input>` when `renamingId` matches; `commitRename` saves via `window.saveProjects`; `focusOnMount` action auto-focuses; exposes `window.startInlineRename`/`window.showInlineDelete` in `onMount`; `projects-service.js` 60-line DOM-manipulation versions removed; `input.card-name` CSS rule added to `style.css`. v0.7.41.

**Phase 10j — COMPLETE** — Remove legacy `#bar` from `index.html`, complete `CanvasBar.svelte`: added `zoomToSelection`, `toggleBookmarkPanel`, `toggleImportPanel`, `pickProjectDir`, `openProjectDirInExplorer`, `exportSharedCanvas` buttons; win-controls wiring moved from `main.js` into `CanvasBar.svelte` `onMount`; dead `syncUndoButtons()` removed from `Canvas.svelte`; two dead `#project-title` DOM writes removed from `projects-service.js`. v0.7.40.

**Phase 10i — COMPLETE** — Delete all remaining dead legacy files: `legacy/storage.js` (586 lines, replaced by `projects-service.js` in Phase 9a), `src/storage.js`, `src/editor.js`, `src/tauri-mock.js` (orphaned top-level copies from pre-migration era). `legacy/` now contains only `tauri-mock.js` (active browser dev mock). -1661 lines. v0.7.39.

**Phase 10h — COMPLETE** — Delete `canvas-stub.js` and `images.js`: `canvas-stub.js` deleted (70 lines), its no-op stubs (`syncUndoButtons`, `updateSelBar`, `addRelHandle`, `saveCanvasState`, etc.) moved into `canvas-actions.js` window block; `images.js` deleted (87 lines), media bridge re-exports and Svelte stub globals inlined directly into `main.js`; `canvas-actions.js` imports `scaleStore`/`pxStore`/`pyStore` from stores directly (no longer depends on `window.scale/px/py` getters). No legacy module imports remain in `main.js` except `tauri-mock.js`. -166 lines. v0.7.38.

**Phase 10g — COMPLETE** — Extract all real logic from `images.js` into `src/lib/canvas-actions.js`: `exportSharedCanvas`, `importSharedCanvas`, `_applySharedCanvas`, `saveCanvasToFile`, `toggleTheme`, `toggleAlwaysOnTop`, `toggleBrainstorm`, `clearBrainstorm`, `addDrawCard`, `gridSelected`, `changeSelectedFontSize`, `togglePin/LockSelected`, `doZoom`, `toggleSnap`, `alignSelFrame`, `distributeSelFrame`, note/frame ctx menu delegates; `images.js` gutted to thin bootstrap (87 lines, stubs + media re-exports only). v0.7.37.

**Phase 10f — COMPLETE** — Remove `#stroke-ctx-menu`, `#summary-panel`, `#export-panel` DOM blocks from `index.html` and corresponding CSS from `style.css` (~100+ lines). -257 lines total. v0.7.36.

**Phase 10e — COMPLETE** — Remove `#toolbar`, `#sel-bar`, `#status`, `#ms-scale-box` DOM blocks from `index.html` and corresponding CSS from `style.css` (~90 lines). -190 lines total. v0.7.35.

**Phase 10d — COMPLETE** — Remove `#cmd-palette` and `#brainstorm-panel` DOM blocks from `index.html` and corresponding CSS from `style.css` (~188 lines). -224 lines total. v0.7.34.

**Phase 10c — COMPLETE** — `SearchBox.svelte` replaces `#search-box` DOM block (reactive search over `elementsStore`, keyboard nav, exposes legacy globals); `ClearConfirm.svelte` replaces `#clear-confirm` DOM block (modal overlay with cancel/confirm); both mounted at body level in `main.js`; `#note-menu`, `#frame-menu`, `#search-box`, `#clear-confirm`, `#minimap` (DOM copy), `#bookmark-panel` (DOM copy), `#radial-menu` DOM blocks removed from `index.html`; corresponding dead CSS (~120 lines) removed from `style.css` (radial-menu, note-menu, frame-menu, search-box, minimap, clear-confirm); `images.js` gutted further — note/frame menu DOM functions replaced with no-op stubs, search/clear functions removed (Svelte components override globals in `onMount`). v0.7.32 → v0.7.33.

**Phase 10b — COMPLETE** — `media-service.js` extracts all blob storage, EXR parser, and media placement logic from `images.js` (~600 lines moved); `MediaDropHandler.svelte` handles clipboard paste + Tauri drag-drop + browser drag-drop events; `ImportPanel.svelte` replaces the legacy `#import-panel` DOM block (positioned dropdown with file-type picker); `TimerPanel.svelte` replaces `#timer-box` DOM block (fully reactive countdown with preset buttons + AudioContext beep); Tauri window controls + auto-updater moved from `images.js` to `main.js`; `images.js` gutted to a thin bootstrap (~300 lines, imports from `media-service.js`); `#import-panel`, `#timer-box`, and legacy hidden file inputs removed from `index.html`. v0.7.31 → v0.7.32.
